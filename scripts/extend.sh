...
#psql -U postgres -d flashback -c "create temp table temp_blocks (t_content text, t_type flashback.block_type, t_language varchar(10)); call create_note('Resource Test', 'Section Test', 'Heading Test');"
#---

#---
psql -U postgres -d flashback -f "$( readlink -f "$(dirname "$0")/.." )/design/create-resource.sql"
#psql -U postgres -d flashback -c "create temp table temp_sections (t_headline varchar(100), t_reference varchar(2000)); call create_resource('Resource Test', 'website', 'Reference Test');"
#---

#---
psql -U postgres -d flashback -f "$( readlink -f "$(dirname "$0")/.." )/design/create-practice.sql"
#psql -U postgres -d flashback -c "create temp table temp_blocks (t_content text, t_type flashback.block_type, t_language varchar(10)); call create_practice('Subject Test', 'Topic Test', 'Practice Heading Test');"
#---
